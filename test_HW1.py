...
class TestHW1(unittest.TestCase):
  ...
  def test2(self):
    res = requests.post('http://localhost:'+str(PORT) +
                        '/test?msg=HoorayAMessage123')
    self.assertEqual(res.text, 'POST message received: HoorayAMessage123',
  ...
  def test4(self):
    ...
    self.assertEqual(res.status_code, 200,
                     msg='Did not return status 200 to GET request to /hello endpoint')
    res = requests.post('http://localhost:'+str(PORT)+'/hello')
    self.assertEqual(res.status_code, 405,
                     msg='Did not return status 405 to POST request to /hello endpoint')
    res = requests.get('http://localhost:'+str(PORT)+'/test')
    self.assertEqual(res.status_code, 200,
                     msg='Did not return status 200 to GET request to /test endpoint')

    res = requests.post('http://localhost:'+str(PORT)+'/test')
    self.assertEqual(res.status_code, 200,
                     msg='Did not return status 200 to POST request to /test endpoint')
